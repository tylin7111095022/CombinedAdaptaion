...
from .IN_Unet import Teacher_UNet, Student_UNet
import torch

def get_models(model_name:str, is_cls:bool, args):
    """option: in_unet, bn_unet"""
    if model_name == "teacher_unet":
        model = Teacher_UNet(n_channels=args.in_channel,n_classes=args.classes,is_normalize=args.is_normalize,is_cls=is_cls,pad_mode=args.pad_mode,instance_branch=False)
    elif model_name == "student_unet":
        model = Student_UNet(n_channels=args.in_channel,n_classes=args.classes,is_normalize=args.is_normalize,is_cls=is_cls,pad_mode=args.pad_mode,instance_branch=False)
    else:
...
if __name__ == '__main__':
    ...
    student = Teacher_UNet(n_channels=1, n_classes=2, is_cls=True)
    teacher = Teacher_UNet(n_channels=1, n_classes=2,is_proj=False, is_cls=True)
